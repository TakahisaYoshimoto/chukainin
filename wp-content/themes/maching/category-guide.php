<?php get_header(); ?>
<div class="l_main l_sub">
	<div class="p_inner2 p_flex p_jc_between">

		<div class="l_content">

			<div class="p_breadcrumbs">
				<ul class="p_flex">
					<li><a href="./index.html">HOME</a></li>
					<li>ご利用ガイド</li>
				</ul>
			</div>

			<div class="p_heading">
				<h2>ご利用ガイド</h2>
			</div>

			<div class="c_guide_ttl">
				<img src="./img/guide_ttl.png">
			</div>

			<div class="c_guide_step_frame">
				<ul>
					<li>
						<a id="area1" name="area1"></a>
						<div class="c_guide_step_txt">
							<h2>登録頂きました投資家様の情報につきましては非公開情報となっております。</h2>
							投資家様から頂きました情報につきましては厳重に保護され他の投資家ユーザー、エージェントユーザーは閲覧できないシステムとなっておりますのでご安心下さい。<br>
							投資家様の情報を閲覧できるのは投資家様がオファー依頼を出されたエージェントのみとなっております。<br>
							また、投資家様からエージェントへオファーにつきましても投資家様の氏名、勤務先、世帯年収、連絡先などはエージェントがオファー承諾しない限り表示されません。<br>
							それゆえに投資家様からエージェントへのオファーメッセッージにおいてはエージェントのオファー受託の大きなポイントになりますのでできるだけ詳細なメーセージをご記入下さいますようお願い致します。<br>
						</div>
						<div class="c_guide_step_summary">
							SSLによる暗号化通信を採用。<br>
							投資家様の個人情報は厳重に保護されます。情報漏えいのご心配はありません。<br>
						</div>
					</li>

					<li>
						<a id="area2" name="area2"></a>
						<div class="c_guide_step_txt">
							<h2>物件探しから収益物件売買に特化したエージェント探しへ</h2>
							収益物件がポータルサイトに掲載されるまでには必ずエージェントが介在して物件掲載されます。<br>
							視点を少し変えて物件探しからエージェント探しへ先回りする事により多くの優良物件が獲得できる確率は格段に向上します。<br>
							また、優良物件は大きな不動産業者、経験豊富なエージェントのみに集まってくる訳ではありません。<br>
							経験は浅くとも日々現場の第一線で活動するエージェントや会社の規模は小さくとも古くから地元へ根付き地縁ある業者の元へも集まって来るのです。<br>
							ご自分の投資スタイルに該当するエージェントを検索して一人でも多くのエージェントと繋がりましょう。<br>
						</div>
						<div class="c_guide_step_summary">
							購入検討地域から・購入希望物件種別から・各種新着情報から、<br>
							これらの機能をうまく活用してあなたにピッタリなエージェントを探しましょう。
						</div>
					</li>

					<li>
						<a id="area3" name="area3"></a>
						<div class="c_guide_step_txt">
							<h2>優良物件情報を入手すればすぐに連絡が来るように事前紹介オファーを出しましょう。</h2>
							数年前までは大手ポータルサイトにも優良物件は数多く掲載されていました。しかし、多くの投資家が収益物件市場へ参入している現在においては稀に優良物件がポータルサイトに掲載されても一瞬で多くの投資家様からお問合わせが入りあっという間に何本もの買い付けが入る状況にあります。<br>
							投資家様は今までのように日々ポータルサイトをチェックするだけでは優良物件情報は獲得できない市況状況にあります。<br>
							本サイトをご利用しご自分の投資スタイルに該当する物件が出たらすぐに連絡を貰えるように事前紹介オファーを出しましょう。<br>
							オファー依頼を出される本数に比例して投資家様のもとへ今までの何倍もの情報が集まりより効率的にそして収益物件への投資事業をよりスピートアップさせます。<br>
							まずは、ご登録後には購入条件に該当するすべてのエージェントに一括オファーを出される事をお薦め致します。<br>
							優良物件情報を獲得する為にはより行動的に積極的なアプローチが大切です。
						</div>
						<div class="c_guide_step_summary">
							幅広い情報発信こそが優良物件情報獲得のポイント。<br>
							一括オファーを活用して一人でも多くのエージェントにオファーを出しましょう。
						</div>
					</li>

					<li>
						<a id="area4" name="area4"></a>
						<div class="c_guide_step_txt">
							<h2>エージェントより未公開物件情報がぞくぞくと投資家様へ</h2>
							一括オファーをお使いになり多くのエージェントへと事前紹介オファーをだされましたら投資家様がご指定さなれた連絡方法によって未公開物件情報がぞくぞくと投資家様の元へ集まります。<br>
							しかし、中には未公開物件情報といえ優良物件では無いものや見当違いの物件紹介もあるかと存じます。<br>
							投資家様におかれましては川上で物件情報を得る事のメリットをご理解の上で物件情報を提供してきたエージェントへ紹介物件に買い付けをださない理由をご返信する事でミスマッチを防ぎまた、相互理解が深化しより良い物件情報が得られやすくなりますのでご面倒でも物件紹介エージェントへのご返信下さいますようお願い致します。<br>
						</div>
						<div class="c_guide_step_summary">
							手間の掛かる物件探しから開放。<br>
							あとは、あなたの専属エージェントからの未公開収益物件情報を待つだけ！
						</div>
					</li>
				</ul>
				<div class="c_guide_step_frame_bg"></div>
			</div>

		</div>

		<div class="l_sidebar">
			<div class="l_side_content">
				<a href="http://www.abc-planning.sakura.ne.jp/lp/002/" target="_blank"><img src="./img/side_banner1.png"></a>
			</div>
			<div class="l_side_content">
				<a href="http://www.abc-planning.sakura.ne.jp/lp/001/" target="_blank"><img src="./img/side_banner2.png"></a>
			</div>
			<div class="l_side_content">
				<div class="l_side_reg">
					<div class="l_side_reg_title">新規登録</div>
					<div class="l_side_reg_body">
						<div class="p_bt p_yellow p_shadow2">
							<a href="">
								無料で新規登録<br>
								<span>たった30秒で完了!!</span>
							</a>
						</div>
					</div>
				</div>
			</div>
			<div class="l_side_content">
				<a href=""><img src="./img/side_banner3.png"></a>
			</div>
			<div class="l_side_content">
				<div class="l_side_login_button p_bt p_green p_shadow2">
					<a href="">ログインはこちら</a>
				</div>
			</div>
			<div class="l_side_content">
				<a href=""><img src="./img/side_second_opinion.png"></a>
			</div>
			<div class="l_side_content">
				<a href="./second_opinion.html"><img src="./img/side_second_opinion.png"></a>
			</div>
			<div class="l_side_content">
				<a href="./list_property.html"><img src="./img/side_undisclosed_property.png"></a>
			</div>
			<div class="l_side_content">
				<a href="./list_success.html"><img src="./img/side_success_stories.png"></a>
			</div>
			<div class="l_side_content">
				<div class="l_side_notice">
					<div class="l_side_notice_title">運営からのお知らせ</div>
					<div class="l_side_notice_body">
						<ul>
							<li>
								<div class="l_side_notice_body_d">2017/10/01</div>
								<div class="l_side_notice_body_t">「未公開収益物件仲介人」PC版をリリースしました。</div>
							</li>
							<li>
								<div class="l_side_notice_body_d">2017/10/01</div>
								<div class="l_side_notice_body_t">「未公開収益物件仲介人」PC版をリリースしました。</div>
							</li>
							<li>
								<div class="l_side_notice_body_d">2017/10/01</div>
								<div class="l_side_notice_body_t">「未公開収益物件仲介人」PC版をリリースしました。</div>
							</li>
							<li>
								<div class="l_side_notice_body_d">2017/10/01</div>
								<div class="l_side_notice_body_t">「未公開収益物件仲介人」PC版をリリースしました。</div>
							</li>
							<li>
								<div class="l_side_notice_body_d">2017/10/01</div>
								<div class="l_side_notice_body_t">「未公開収益物件仲介人」PC版をリリースしました。</div>
							</li>
							<li>
								<div class="l_side_notice_body_d">2017/10/01</div>
								<div class="l_side_notice_body_t">「未公開収益物件仲介人」PC版をリリースしました。</div>
							</li>
							<li>
								<div class="l_side_notice_body_d">2017/10/01</div>
								<div class="l_side_notice_body_t">「未公開収益物件仲介人」PC版をリリースしました。</div>
							</li>
							<li>
								<div class="l_side_notice_body_d">2017/10/01</div>
								<div class="l_side_notice_body_t">「未公開収益物件仲介人」PC版をリリースしました。</div>
							</li>
							<li>
								<div class="l_side_notice_body_d">2017/10/01</div>
								<div class="l_side_notice_body_t">「未公開収益物件仲介人」PC版をリリースしました。</div>
							</li>
							<li>
								<div class="l_side_notice_body_d">2017/10/01</div>
								<div class="l_side_notice_body_t">「未公開収益物件仲介人」PC版をリリースしました。</div>
							</li>
						</ul>
					</div>
				</div>
			</div>

			<div class="l_sidebar">
				<div class="l_side_search">
					<div class="l_side_search_title"><img src="./img/icon_search_g.png">地域から探す</div>
					<div class="l_side_search_body">
						<dl class="p_flex">
							<dt>北海道</dt>
							<dd>
								<ul class="p_flex">
									<li><a href="./list_search_result.html">北海道</a></li>
								</ul>
							</dd>
							<dt>東北</dt>
							<dd>
								<ul class="p_flex">
									<li><a href="./list_search_result.html">青森</a></li>
									<li><a href="./list_search_result.html">岩手</a></li>
									<li><a href="./list_search_result.html">宮城</a></li>
									<li><a href="./list_search_result.html">秋田</a></li>
									<li><a href="./list_search_result.html">山形</a></li>
									<li><a href="./list_search_result.html">福島</a></li>
								</ul>
							</dd>
							<dt>関東</dt>
							<dd>
								<ul class="p_flex">
									<li><a href="./list_search_result.html">茨城</a></li>
									<li><a href="./list_search_result.html">栃木</a></li>
									<li><a href="./list_search_result.html">群馬</a></li>
									<li><a href="./list_search_result.html">埼玉</a></li>
									<li><a href="./list_search_result.html">千葉</a></li>
									<li><a href="./list_search_result.html">東京</a></li>
									<li><a href="./list_search_result.html">神奈川</a></li>
								</ul>
							</dd>
							<dt>北陸</dt>
							<dd>
								<ul class="p_flex">
									<li><a href="./list_search_result.html">新潟</a></li>
									<li><a href="./list_search_result.html">富山</a></li>
									<li><a href="./list_search_result.html">石川</a></li>
									<li><a href="./list_search_result.html">福井</a></li>
								</ul>
							</dd>
							<dt>中部</dt>
							<dd>
								<ul class="p_flex">
									<li><a href="./list_search_result.html">山梨</a></li>
									<li><a href="./list_search_result.html">長野</a></li>
									<li><a href="./list_search_result.html">岐阜</a></li>
									<li><a href="./list_search_result.html">静岡</a></li>
									<li><a href="./list_search_result.html">愛知</a></li>
									<li><a href="./list_search_result.html">三重</a></li>
								</ul>
							</dd>
							<dt>関西</dt>
							<dd>
								<ul class="p_flex">
									<li><a href="./list_search_result.html">滋賀</a></li>
									<li><a href="./list_search_result.html">京都</a></li>
									<li><a href="./list_search_result.html">大阪</a></li>
									<li><a href="./list_search_result.html">兵庫</a></li>
									<li><a href="./list_search_result.html">奈良</a></li>
									<li><a href="./list_search_result.html">和歌山</a></li>
								</ul>
							</dd>
							<dt>中国</dt>
							<dd>
								<ul class="p_flex">
									<li><a href="./list_search_result.html">鳥取</a></li>
									<li><a href="./list_search_result.html">島根</a></li>
									<li><a href="./list_search_result.html">岡山</a></li>
									<li><a href="./list_search_result.html">広島</a></li>
									<li><a href="./list_search_result.html">山口</a></li>
								</ul>
							</dd>
							<dt>四国</dt>
							<dd>
								<ul class="p_flex">
									<li><a href="./list_search_result.html">徳島</a></li>
									<li><a href="./list_search_result.html">香川</a></li>
									<li><a href="./list_search_result.html">愛媛</a></li>
									<li><a href="./list_search_result.html">高知</a></li>
								</ul>
							</dd>
							<dt>九州</dt>
							<dd>
								<ul class="p_flex">
									<li><a href="./list_search_result.html">福岡</a></li>
									<li><a href="./list_search_result.html">佐賀</a></li>
									<li><a href="./list_search_result.html">長崎</a></li>
									<li><a href="./list_search_result.html">熊本</a></li>
									<li><a href="./list_search_result.html">大分</a></li>
									<li><a href="./list_search_result.html">宮崎</a></li>
									<li><a href="./list_search_result.html">鹿児島</a></li>
								</ul>
							</dd>
							<dt>沖縄</dt>
							<dd>
								<ul class="p_flex">
									<li><a href="./list_search_result.html">沖縄</a></li>
								</ul>
							</dd>
						</dl>
					</div>
					<div class="l_side_search_title"><img src="./img/icon_search_g.png">物件種別からさがす</div>
					<div class="l_side_search_body">
						<ul class="p_flex">
							<li><a href="./list_search_result.html">一棟RC</a></li>
							<li><a href="./list_search_result.html">一棟鉄骨</a></li>
							<li><a href="./list_search_result.html">一棟木造</a></li>
							<li><a href="./list_search_result.html">商業ビル</a></li>
							<li><a href="./list_search_result.html">戸建賃貸</a></li>
							<li><a href="./list_search_result.html">区分マンション</a></li>
							<li><a href="./list_search_result.html">土地</a></li>
							<li><a href="./list_search_result.html">駐車場</a></li>
							<li><a href="./list_search_result.html">ホテル</a></li>
							<li><a href="./list_search_result.html">区分事務所</a></li>
							<li><a href="./list_search_result.html">区分店舗</a></li>
							<li><a href="./list_search_result.html">倉庫</a></li>
							<li><a href="./list_search_result.html">工場</a></li>
							<li><a href="./list_search_result.html">太陽光発電用地</a></li>
							<li><a href="./list_search_result.html">区分太陽光発電</a></li>
						</ul>
					</div>
				</div>
			</div>

			<div class="l_sidebar">
				<div class="sns">

				</div>
			</div>

		</div>
	</div>
</div>

<div class="l_footer">
	<div class="p_inner2">
		<div class="p_flex">
			<div class="l_footer_search_area">
				<div class="l_footer_search_title"><img src="./img/icon_search_w_s.png">地域から探す</div>
				<div class="p_flex">
					<div class="l_footer_search_body l_flex">
						<dl class="p_flex">
							<dt>北海道</dt>
							<dd>
								<ul class="p_flex">
									<li><a href="./list_search_result.html">北海道</a></li>
								</ul>
							</dd>
							<dt>東北</dt>
							<dd>
								<ul class="p_flex">
									<li><a href="./list_search_result.html">青森</a></li>
									<li><a href="./list_search_result.html">岩手</a></li>
									<li><a href="./list_search_result.html">宮城</a></li>
									<li><a href="./list_search_result.html">秋田</a></li>
									<li><a href="./list_search_result.html">山形</a></li>
									<li><a href="./list_search_result.html">福島</a></li>
								</ul>
							</dd>
							<dt>関東</dt>
							<dd>
								<ul class="p_flex">
									<li><a href="./list_search_result.html">茨城</a></li>
									<li><a href="./list_search_result.html">栃木</a></li>
									<li><a href="./list_search_result.html">群馬</a></li>
									<li><a href="./list_search_result.html">埼玉</a></li>
									<li><a href="./list_search_result.html">千葉</a></li>
									<li><a href="./list_search_result.html">東京</a></li>
									<li><a href="./list_search_result.html">神奈川</a></li>
								</ul>
							</dd>
							<dt>北陸</dt>
							<dd>
								<ul class="p_flex">
									<li><a href="./list_search_result.html">新潟</a></li>
									<li><a href="./list_search_result.html">富山</a></li>
									<li><a href="./list_search_result.html">石川</a></li>
									<li><a href="./list_search_result.html">福井</a></li>
								</ul>
							</dd>
							<dt>中部</dt>
							<dd>
								<ul class="p_flex">
									<li><a href="./list_search_result.html./list_search_result.html">山梨</a></li>
									<li><a href="./list_search_result.html">長野</a></li>
									<li><a href="./list_search_result.html">岐阜</a></li>
									<li><a href="./list_search_result.html">静岡</a></li>
									<li><a href="./list_search_result.html">愛知</a></li>
									<li><a href="./list_search_result.html">三重</a></li>
								</ul>
							</dd>
						</dl>
					</div>
					<div class="l_footer_search_body l_flex">
						<dl class="p_flex">
							<dt>関西</dt>
							<dd>
								<ul class="p_flex">
									<li><a href="./list_search_result.html">滋賀</a></li>
									<li><a href="./list_search_result.html">京都</a></li>
									<li><a href="./list_search_result.html">大阪</a></li>
									<li><a href="./list_search_result.html">兵庫</a></li>
									<li><a href="./list_search_result.html">奈良</a></li>
									<li><a href="./list_search_result.html">和歌山</a></li>
								</ul>
							</dd>
							<dt>中国</dt>
							<dd>
								<ul class="p_flex">
									<li><a href="./list_search_result.html">鳥取</a></li>
									<li><a href="./list_search_result.html">島根</a></li>
									<li><a href="./list_search_result.html">岡山</a></li>
									<li><a href="./list_search_result.html">広島</a></li>
									<li><a href="./list_search_result.html">山口</a></li>
								</ul>
							</dd>
							<dt>四国</dt>
							<dd>
								<ul class="p_flex">
									<li><a href="./list_search_result.html">徳島</a></li>
									<li><a href="./list_search_result.html">香川</a></li>
									<li><a href="./list_search_result.html">愛媛</a></li>
									<li><a href="./list_search_result.html">高知</a></li>
								</ul>
							</dd>
							<dt>九州</dt>
							<dd>
								<ul class="p_flex">
									<li><a href="./list_search_result.html">福岡</a></li>
									<li><a href="./list_search_result.html">佐賀</a></li>
									<li><a href="./list_search_result.html">長崎</a></li>
									<li><a href="./list_search_result.html">熊本</a></li>
									<li><a href="./list_search_result.html">大分</a></li>
									<li><a href="./list_search_result.html">宮崎</a></li>
									<li><a href="./list_search_result.html">鹿児島</a></li>
								</ul>
							</dd>
							<dt>沖縄</dt>
							<dd>
								<ul class="p_flex">
									<li><a href="./list_search_result.html">沖縄</a></li>
								</ul>
							</dd>
						</dl>
					</div>
				</div>
			</div>
			<div class="l_footer_search_property">
				<div class="l_footer_search_title"><img src="./img/icon_search_w_s.png">物件種別からさがす</div>
				<div class="l_footer_search_body">
					<ul class="p_flex">
						<li><a href="./list_search_result.html">一棟RC</a></li>
						<li><a href="./list_search_result.html">一棟鉄骨</a></li>
						<li><a href="./list_search_result.html">一棟木造</a></li>
						<li><a href="./list_search_result.html">商業ビル</a></li>
						<li><a href="./list_search_result.html">戸建賃貸</a></li>
						<li><a href="./list_search_result.html">区分マンション</a></li>
						<li><a href="./list_search_result.html">土地</a></li>
						<li><a href="./list_search_result.html">駐車場</a></li>
						<li><a href="./list_search_result.html">ホテル</a></li>
						<li><a href="./list_search_result.html">区分事務所</a></li>
						<li><a href="./list_search_result.html">区分店舗</a></li>
						<li><a href="./list_search_result.html">倉庫</a></li>
						<li><a href="./list_search_result.html">工場</a></li>
						<li><a href="./list_search_result.html">太陽光発電用地</a></li>
						<li><a href="./list_search_result.html">区分太陽光発電</a></li>
					</ul>
				</div>
			</div>
		</div>
	</div>
<?php get_footer(); ?>
</div>
</div>
<script type="text/javascript" charset="UTF-8" src="./js/jquery-3.2.1.min.js"></script>
<script type="text/javascript" charset="UTF-8" src="./js/contents.js"></script>
</body>
</html>
